...
void Database::parse_datapack(const String &_datapack_title) {
    ...
    for (int i = 0; i < _block_count; i++) {
        ...
        if (_block["material"] != "") {
            ...
            Dictionary _block_mat_info = JSON::parse_string(_block_mat_text);
            StandardMaterial3D *_mat = memnew(StandardMaterial3D);

            _mat->set_texture(StandardMaterial3D::TEXTURE_ALBEDO, load_texture(_pack_path + String(_block_mat_info["albedo"])));
            _mat->set_texture_filter((StandardMaterial3D::TextureFilter)int(_block["texture_filter"]));
